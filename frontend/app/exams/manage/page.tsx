...
import { Exam } from "../types"; // Lưu ý đường dẫn import types có thể thay đổi tùy cấu trúc folder (../../)
...
export default function ManageExamsPage() {
  ...
  const handleSoftDelete = async (examId: number) => {
    ...
    try {
      // Gọi API DELETE (Backend đã sửa thành set isActive = false)
      const res = await fetch(`http://localhost:8080/api/exams/${examId}`, {
        method: "DELETE",
      });

      if (res.ok) {
        // CẬP NHẬT UI: Không xóa khỏi list, mà đổi trạng thái isActive thành false
        setExams((prevExams) => 
            prevExams.map((exam) => 
        ...
        {exams.length === 0 ? (
            ...
            <div key={exam.examId} className={`border p-5 rounded-lg shadow-sm transition flex justify-between items-center ${exam.isActive ? 'bg-white' : 'bg-gray-100'}`}>
                ...
                <div className="flex gap-2">
                    ...
                    {/* CHỈ HIỆN NÚT ẨN KHI BÀI ĐANG ACTIVE */}
                    {exam.isActive && (
                        <button 
