  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        {/* --- SỬA Ở ĐÂY: NÚT QUẢN LÝ --- */}
        {role === "TEACHER" && (
          <button
            onClick={() => router.push("/exams/manage")} // Chuyển sang trang quản lý
      ...
      <div className="space-y-4">
        {exams?.map((exam, index) => {
            ...
            const isOpen = exam.isActive && !isExpired;
                    ...
                    <div className="flex flex-wrap gap-3 mt-3 text-sm">
                      ...
                      {hasEndTime && (
                         <span className={`px-2 py-1 rounded border ${isExpired ? 'bg-red-50 text-red-600 border-red-200' : 'bg-yellow-50 text-yellow-700 border-yellow-200'}`}>
                            Hết hạn: {formatDate(exam.endTime!)}
                         </span>
                      )}
                      <span className={`px-2 py-1 rounded font-bold border ${isOpen ? 'bg-green-100 text-green-700 border-green-200' : 'bg-gray-200 text-gray-600 border-gray-300'}`}>
                         {isOpen ? 'Đang mở' : (isExpired ? 'Đã kết thúc' : 'Đã đóng')}
                      </span>
                    </div>
                  </div>
                  <button
                    ...
                    `}
                  >
                    {isOpen ? 'Làm bài' : (isExpired ? 'Hết hạn' : 'Đã khóa')}
                  </button>
                </div>
              </div>
            );
