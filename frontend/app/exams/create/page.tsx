...
export default function CreateExamPage() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setDraftQ((prev) => ({ ...prev, image: reader.result as string }));
      };
  ...
  const removeDraftOption = (id: number) => {
    setDraftQ((prev) => ({
      ...prev,
  ...
  // --- LOGIC LƯU CÂU HỎI VÀO DANH SÁCH (THÊM HOẶC SỬA) ---
  const saveQuestionToList = () => {
    ...
    // Nếu id = 0 -> Tạo ID mới (Thêm mới)
    // Nếu id != 0 -> Giữ nguyên ID (Cập nhật)
    const questionId = draftQ.id === 0 ? Date.now() : draftQ.id;
    const questionToSave = { ...draftQ, id: questionId };

    // Kiểm tra xem ID này đã có trong list chưa
  ...
  // --- LOGIC SỬA CÂU HỎI TỪ DANH SÁCH ---
  const editQuestionFromList = (q: DraftQuestion) => {
    setDraftQ(q); // Đổ dữ liệu vào form
    setIsAddingQuestion(true); // Mở form
  };

  const removeQuestionFromList = (idToDelete: number) => {
  ...
  const handleSubmitExam = async () => {
    ...
    try {
      ...
      router.push("/exams/manage"); // Chuyển hướng về trang quản lý
    } catch (error) {
      console.error(error);
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          {isAddingQuestion && (
            <div className="bg-blue-50 border-2 border-blue-200 p-6 rounded-lg shadow-md animate-fade-in">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleQuestionTypeChange(e.target.value as "MCQ" | "ESSAY")}
                    className="w-full border p-2 rounded"
                  >
              ...
              {/* PHẦN OPTIONS - ĐẦY ĐỦ LOGIC NHƯ TRANG EDIT */}
